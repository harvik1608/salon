...
(function (factory) {
  ...
  var COL_MIN_WIDTH,
    ...
    UPGRADE_WINDOW,
    VRowGroup,
    VertResourceViewMixin,
    View,
    _filterResourcesWithEvents,
    applyAll,
    capitaliseFirstLetter,
    compareByFieldSpecs,
    computeIntervalUnit,
    computeOffsetForSeg,
    computeOffsetForSegs,
    copyRect,
    createObject,
    cssToStr,
    debounce,
    detectWarningInContainer,
    ...
    joinRects,
    multiplyDuration,
    origExecuteEventsRender,
    origGetSegCustomClasses,
    origGetSegDefaultBackgroundColor,
    origGetSegDefaultBorderColor,
    origGetSegDefaultTextColor,
    origHandleDate,
    origOnDateRender,
    origRemoveElement,
    origSetElement,
  ...
  if (FC.internalApiVersion !== 7) {
    FC.warn(
      "v" +
        FC.schedulerVersion +
        " of FullCalendar Scheduler " +
        "is incompatible with v" +
        FC.version +
        " of the core.\n" +
 ...
 */
  ...
  EnhancedScroller = (function (superClass) {
    ...
    EnhancedScroller.prototype.bindPreventTouchScroll = function () {
      if (!this.preventTouchScrollHandler) {
        return this.scrollEl.on(
          "touchmove",
          (this.preventTouchScrollHandler = FC.preventDefault)
        );
      }
    };
   ...
   */
    ...
    detectRtlScrollSystem = function () {
      var el, node, system;
      el = $(
        '<div style=" position: absolute top: -1000px; width: 1px; height: 1px; overflow: scroll; direction: rtl; font-size: 14px; ">A</div>'
      ).appendTo("body");
      node = el[0];
      system =
        node.scrollLeft > 0
          ? "positive"
          : ((node.scrollLeft = 1), el.scrollLeft > 0 ? "reverse" : "negative");
      el.remove();
      return system;
    };
   ...
   */
    ...
    ClippedScroller.prototype.updateSize = function () {
      ...
      return scrollEl.toggleClass(
        "fc-no-scrollbars",
        (this.isHScrollbarsClipped || this.overflowX === "hidden") &&
          (this.isVScrollbarsClipped || this.overflowY === "hidden") &&
          !(
            scrollbarWidths.top ||
 ...
 */
                ...
                for (j = 0, len = ref.length; j < len; j++) {
                  ...
                  if (otherScroller !== scroller) {
                    switch (_this.axis) {
                      case "horizontal":
                        results.push(
                          otherScroller.setNativeScrollLeft(
                            scroller.getNativeScrollLeft()
                          )
                        );
                        break;
                      case "vertical":
  ...
  ScrollFollower = (function () {
    ...
    function ScrollFollower(scroller, allowPointerEvents) {
      this.allowPointerEvents =
        allowPointerEvents != null ? allowPointerEvents : false;
  ...
  ScrollFollowerSprite = (function () {
    ...
    ScrollFollowerSprite.prototype.cacheDimensions = function () {
      ...
      containerRect = this.containerRect = joinRects(
        follower.getContentRect(parentEl),
        naturalRect
      );
      ...
      if (follower.isHFollowing) {
        if (
          getRectWidth(containerRect) - getRectWidth(naturalRect) >=
          minTravel
        ) {
      ...
      if (follower.isVFollowing) {
        if (
          getRectHeight(containerRect) - getRectHeight(naturalRect) >=
          minTravel
        ) {
          isVFollowing = true;
        }
    ...
    ScrollFollowerSprite.prototype.computePosition = function () {
      ...
      if (visibleParentRect) {
        ...
        if (
          (this.isCentered && !testRectContains(viewportRect, parentRect)) ||
          ...
          if (this.isHFollowing) {
            if (this.isCentered) {
              ...
              rect.left =
                (visibleParentRect.left + visibleParentRect.right) / 2 -
                rectWidth / 2;
  ...
  testRectContains = function (rect, innerRect) {
    return (
      testRectHContains(rect, innerRect) && testRectVContains(rect, innerRect)
    );
  };
  ...
  CalendarExtension = (function (superClass) {
    ...
    CalendarExtension.prototype.isSpanAllowed = function (span, constraint) {
      ...
      if (typeof constraint === "object") {
        ...
        if (
          constrainToResourceIds.length &&
          (!span.resourceId ||
            !((ref = span.resourceId),
            indexOf.call(constrainToResourceIds, ref) >= 0))
        ) {
          return false;
        }
      }
    ...
    CalendarExtension.prototype.getPeerEvents = function (span, event) {
      var filteredPeerEvents,
        ...
        peerResourceId,
        peerResourceIds;
      peerEvents = CalendarExtension.__super__.getPeerEvents.apply(
        this,
        arguments
      );
      newResourceIds = span.resourceId
        ? [span.resourceId]
        : event
        ? this.getEventResourceIds(event)
        : [];
      filteredPeerEvents = [];
    ...
    CalendarExtension.prototype.spanContainsSpan = function (
      outerSpan,
      innerSpan
    ) {
      if (
        outerSpan.resourceId &&
        outerSpan.resourceId !== innerSpan.resourceId
      ) {
        return false;
      } else {
        return CalendarExtension.__super__.spanContainsSpan.apply(
          this,
          arguments
        );
      }
    ...
    CalendarExtension.prototype.getCurrentBusinessHourEvents = function (
      wholeDay
    ) {
      var allEvents,
        anyCustomBusinessHours,
        event,
        events,
        flatResources,
        j,
      ...
      if (anyCustomBusinessHours) {
        ...
        for (k = 0, len1 = flatResources.length; k < len1; k++) {
          ...
          events = this.computeBusinessHourEvents(
            wholeDay,
            resource.businessHours || this.options.businessHours
          );
        ...
        return CalendarExtension.__super__.getCurrentBusinessHourEvents.apply(
          this,
          arguments
        );
      }
    ...
    CalendarExtension.prototype.buildSelectSpan = function (
      startInput,
      endInput,
      resourceId
    ) {
    ...
    CalendarExtension.prototype.getEventResourceIds = function (event) {
      ...
      resourceId = String(
        (ref =
          (ref1 = event[this.getEventResourceField()]) != null
            ? ref1
            : event.resourceId) != null
          ? ref
          : ""
    ...
    CalendarExtension.prototype.setEventResourceId = function (
      event,
      resourceId
    ) {
      return (event[this.getEventResourceField()] = String(
        ...
        return this.clientEvents(
          (function (_this) {
            return function (event) {
              return (
                $.inArray(resource.id, _this.getEventResourceIds(event)) !== -1
              );
            };
          })(this)
        );
  ...
  View.prototype.executeEventsRender = function (events) {
    return this.whenResourcesSet().then(
      (function (_this) {
        return function () {
  ...
  View.prototype.bindResources = function () {
    ...
    if (!this.isResourcesBound) {
      ...
      promise = this.opt("refetchResourcesOnNavigate")
        ? this.fetchResources()
        : this.requestResources();
      return this.rejectOn("resourcesUnbind", promise).then(
        (function (_this) {
  ...
  origGetSegDefaultBackgroundColor =
    Grid.prototype.getSegDefaultBackgroundColor;
  ...
  Grid.prototype.getSegDefaultBackgroundColor = function (seg) {
    ...
    for (j = 0, len = resources.length; j < len; j++) {
      ...
      while (currentResource) {
        val =
          currentResource.eventBackgroundColor || currentResource.eventColor;
        if (val) {
          return val;
        }
  ...
  ResourceManager = (function (superClass) {
    ...
    ResourceManager.prototype.fetchResources = function () {
      ...
      return (this.fetching = new Promise(
        (function (_this) {
    ...
    ResourceManager.prototype.fetchResourceInputs = function (callback) {
      ...
      switch ($.type(source)) {
        ...
        case "object":
          this.calendar.pushLoading();
          return $.ajax(
            $.extend({}, ResourceManager.ajaxDefaults, source)
          ).then(
            (function (_this) {
              return function (resourceInputs) {
                _this.calendar.popLoading();
    ...
    ResourceManager.prototype.setResources = function (resourceInputs) {
      ...
      return this.calendar.publiclyTrigger(
        "resourcesSet",
        null,
        this.topLevelResources
      );
    ...
    ResourceManager.prototype.removeResourceFromTree = function (
      resource,
      siblings
    ) {
      var i, j, len, sibling;
 ...
 */
  ...
  ResourceViewMixin = {
    ...
    onDateRender: function () {
      return this.rejectOn("dateUnrender", this.whenResourcesRendered()).then(
        (function (_this) {
          return function () {
    ...
    requestResourcesRender: function (resources) {
      return this.resourceRenderQueue.add(
        (function (_this) {
          return function () {
    ...
    executeResourcesRender: function (resources) {
      this.captureScroll();
      this.freezeHeight();
      return this.executeResourcesUnrender().then(
        (function (_this) {
          return function () {
    ...
    executeResourcesUnrender: function (teardownOptions) {
      if (this.isResourcesRendered) {
        return this.requestEventsUnrender().then(
          (function (_this) {
            return function () {
              _this.captureScroll();
              _this.freezeHeight();
    ...
    isEventDraggable: function (event) {
      return (
        this.isEventResourceEditable(event) ||
        View.prototype.isEventDraggable.call(this, event)
      );
    },
    isEventResourceEditable: function (event) {
      ...
      return (ref =
        (ref1 =
          (ref2 = event.resourceEditable) != null
            ? ref2
            : (event.source || {}).resourceEditable) != null
          ? ref1
          : this.opt("eventResourceEditable")) != null
        ? ref
   ...
   */
    reportEventDrop: function () {
      ...
      (event = arguments[0]),
        (dropLocation = arguments[1]),
        (otherArgs = 3 <= arguments.length ? slice.call(arguments, 2) : []);
      ...
      return (ref = View.prototype.reportEventDrop).call.apply(
        ref,
        [this, event, dropLocation].concat(slice.call(otherArgs))
      );
    },
    reportExternalDrop: function () {
      ...
      (meta = arguments[0]),
        ...
        (otherArgs = 3 <= arguments.length ? slice.call(arguments, 2) : []);
      dropLocation = this.normalizeDropLocation(dropLocation);
      return (ref = View.prototype.reportExternalDrop).call.apply(
        ref,
        [this, meta, dropLocation].concat(slice.call(otherArgs))
      );
    },
  ...
  _filterResourcesWithEvents = function (sourceResources, resourceIdHits) {
    ...
    for (j = 0, len = sourceResources.length; j < len; j++) {
      ...
      if (sourceResource.children.length) {
        filteredChildren = _filterResourcesWithEvents(
          sourceResource.children,
          resourceIdHits
        );
 ...
 */
  ...
  VertResourceViewMixin = $.extend({}, ResourceViewMixin, {
    executeResourcesRender: function (resources) {
      ...
      if (this.isDateRendered) {
        return this.requestDateRender().then(
          (function (_this) {
            return function () {
              return _this.reportResourcesRender();
            };
    ...
    executeDateRender: function (date) {
      return View.prototype.executeDateRender.apply(this, arguments).then(
        (function (_this) {
          return function () {
    ...
    executeDateUnrender: function (date) {
      return View.prototype.executeDateUnrender.apply(this, arguments).then(
        (function (_this) {
          return function () {
            if (_this.isResourcesSet) {
              return _this.reportResourcesUnrender();
  ...
  ResourceGridMixin = {
    ...
    computeEventResize: function (type, startSpan, endSpan, event) {
      var resizeLocation;
      if (
        !this.allowCrossResource &&
        startSpan.resourceId !== endSpan.resourceId
      ) {
        return;
      }
    ...
    computeSelectionSpan: function (startSpan, endSpan) {
      ...
      selectionSpan = Grid.prototype.computeSelectionSpan.apply(
        this,
        arguments
      );
      if (selectionSpan) {
 ...
 */
  ...
  ResourceDayTableMixin = {
    ...
    renderHeadResourceAndDateHtml: function () {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        for (
          dayIndex = k = 0, ref1 = this.daysPerRow;
          k < ref1;
          dayIndex = k += 1
        ) {
    ...
    renderHeadDateAndResourceHtml: function () {
      ...
      resourceHtmls = [];
      for (
        dayIndex = j = 0, ref = this.daysPerRow;
        j < ref;
        dayIndex = j += 1
      ) {
        date = this.dayDates[dayIndex].clone();
    ...
    renderHeadResourceCellHtml: function (resource, date, colspan) {
      return (
        ...
        ">" +
        htmlEscape(this.view.getResourceText(resource)) +
        "</th>"
      );
    },
    renderHeadResourceDateCellHtml: function (date, resource, colspan) {
      return this.renderHeadDateCellHtml(
        date,
        colspan,
    ...
    processHeadResourceEls: function (containerEl) {
      return containerEl.find(".fc-resource-cell").each(
        (function (_this) {
          return function (col, node) {
            ...
            if (_this.datesAboveResources) {
              ...
              resource =
                _this.flattenedResources[
                  _this.isRTL ? _this.flattenedResources.length - 1 - col : col
                ];
            }
  ...
  ResourceDayGrid = (function (superClass) {
    ...
    ResourceDayGrid.prototype.spanToSegs = function (span) {
      ...
      if (!resourceCnt) {
        ...
        for (k = 0, len1 = genericSegs.length; k < len1; k++) {
          seg = genericSegs[k];
          for (
            resourceIndex = l = 0, ref = resourceCnt;
            l < ref;
            resourceIndex = l += 1
          ) {
            ...
            if (!span.resourceId || span.resourceId === resourceObj.id) {
              ...
              if (this.isRTL) {
                copy.leftCol = this.indicesToCol(
                  resourceIndex,
                  seg.lastRowDayIndex
                );
                copy.rightCol = this.indicesToCol(
                  resourceIndex,
                  seg.firstRowDayIndex
                );
              } else {
                copy.leftCol = this.indicesToCol(
                  resourceIndex,
    ...
    ResourceDayGrid.prototype.renderBusinessHours = function () {
      ...
      if (segs) {
        ...
        return ResourceDayGrid.__super__.renderBusinessHours.apply(
          this,
          arguments
        );
      }
  ...
  ResourceTimeGrid = (function (superClass) {
    ...
    ResourceTimeGrid.prototype.renderBusinessHours = function () {
      ...
      if (segs) {
        ...
        return ResourceTimeGrid.__super__.renderBusinessHours.apply(
          this,
          arguments
        );
      }
  ...
  TimelineView = (function (superClass) {
    ...
    TimelineView.prototype.renderSkeletonHtml = function () {
      return (
        '<table> <thead class="fc-head"> <tr> <td class="fc-time-area ' +
    ...
    TimelineView.prototype.renderTimeGridSkeleton = function () {
      ...
      return this.timeGrid.bodyScroller.on(
        "scroll",
        proxy(this, "handleBodyScroll")
      );
    };
    ...
    TimelineView.prototype.setHeight = function (totalHeight, isAuto) {
      ...
      if (isAuto) {
        ...
        bodyHeight =
          totalHeight - this.timeGrid.headHeight() - this.queryMiscHeight();
      }
    ...
    TimelineView.prototype.queryMiscHeight = function () {
      return (
        this.el.outerHeight() -
        this.timeGrid.headScroller.el.outerHeight() -
        this.timeGrid.bodyScroller.el.outerHeight()
      );
    };
  ...
  TimelineGrid = (function (superClass) {
    ...
    function TimelineGrid() {
      ...
      this.snapDuration = (input = this.opt("snapDuration"))
        ? moment.duration(input)
        : this.slotDuration;
      this.minResizeDuration = this.snapDuration;
      this.snapsPerSlot = divideDurationByDuration(
        this.slotDuration,
        this.snapDuration
      );
   ...
   */
    ...
    TimelineGrid.prototype.normalizeGridRange = function (range) {
      ...
      if (this.isTimeScale) {
        ...
        if (this.largeUnit) {
          ...
          if (
            !adjustedEnd.isSame(normalRange.end) ||
            !adjustedEnd.isAfter(normalRange.start)
          ) {
    ...
    TimelineGrid.prototype.spanToSegs = function (span) {
      ...
      if (
        this.computeDateSnapCoverage(span.start) <
        this.computeDateSnapCoverage(span.end)
      ) {
        ...
        if (seg) {
          ...
          if (
            seg.isEnd &&
            seg.end &&
            !this.isValidDate(seg.end.clone().subtract(1))
          ) {
            seg.isEnd = false;
          }
          return [seg];
      ...
      if (containerCoordCache.isTopInBounds(topOffset)) {
        ...
        if (slatIndex != null) {
          ...
          if (this.isRTL) {
            ...
            snapLeft =
              snapRight - ((localSnapIndex + 1) / snapsPerSlot) * slatWidth;
          } else {
            ...
            snapRight =
              slatLeft + ((localSnapIndex + 1) / snapsPerSlot) * slatWidth;
          }
          return {
            snap: snapIndex,
            component: this,
    ...
    TimelineGrid.prototype.renderSkeleton = function () {
      ...
      this.slatContainerEl = $('<div class="fc-slats"/>').appendTo(
        this.bodyScroller.canvas.bgEl
      );
      this.segContainerEl = $('<div class="fc-event-container"/>').appendTo(
        this.bodyScroller.canvas.contentEl
      );
      ...
      this.joiner = new ScrollJoiner("horizontal", [
        this.headScroller,
        this.bodyScroller
      ]);
      if (true) {
    ...
    TimelineGrid.prototype.renderDates = function () {
      ...
      if (this.follower) {
        return this.follower.setSprites(
          this.headEl.find("tr:not(:last-child) .fc-cell-text")
        );
      }
    };
      ...
      for (j = 0, len = slotDates.length; j < len; j++) {
        ...
        for (row = k = 0, len1 = formats.length; k < len1; row = ++k) {
          ...
          if (isSuperRow) {
            ...
            if (
              !leadingCell ||
              isInt(divideRangeByDuration(this.start, date, labelInterval))
            ) {
              text = date.format(format);
              newCell = this.buildCellObject(date, text, rowUnits[row]);
            } else {
        ...
        for (n = 0, len4 = rowCells.length; n < len4; n++) {
          ...
          if (isSingleDay) {
            headerCellClassNames = headerCellClassNames.concat(
              this.getDayClasses(cell.date, true)
            );
          }
          html +=
            '<th class="' +
    ...
    TimelineGrid.prototype.slatCellHtml = function (date, isEm) {
      ...
      if (this.isTimeScale) {
        classes = [];
        classes.push(
          isInt(divideRangeByDuration(this.start, date, this.labelInterval))
            ? "fc-major"
            : "fc-minor"
        );
      } else {
    ...
    TimelineGrid.prototype.renderBusinessHours = function () {
      var segs;
      if (!this.largeUnit) {
        segs = this.businessHourSegs = this.buildBusinessHourSegs(
          !this.isTimeScale
        );
        return this.renderFill("businessHours", segs, "bgevent");
      }
      ...
      if (date >= this.start && date < this.end) {
        ...
        nodes.push(
          $("<div class='fc-now-indicator fc-now-indicator-arrow'></div>")
            .css(css)
            .appendTo(this.headScroller.canvas.el)[0]
        );
        nodes.push(
          $("<div class='fc-now-indicator fc-now-indicator-line'></div>")
            .css(css)
            .appendTo(this.bodyScroller.canvas.el)[0]
        );
      }
    ...
    TimelineGrid.prototype.updateWidth = function () {
      ...
      if (isDatesRendered) {
        slotWidth = Math.round(
          this.slotWidth || (this.slotWidth = this.computeSlotWidth())
        );
    ...
    TimelineGrid.prototype.dateToCoord = function (date) {
      ...
      if (this.isRTL) {
        return (
          ...
          this.containerCoordCache.getWidth(0)
        );
      } else {
        return (
    ...
    TimelineGrid.prototype.fgSegHtml = function (seg, disableResizing) {
      ...
      isResizableFromStart =
        seg.isStart && this.view.isEventResizableFromStart(event);
      isResizableFromEnd =
        seg.isEnd && this.view.isEventResizableFromEnd(event);
      classes = this.getSegClasses(
        seg,
        isDraggable,
    ...
    TimelineGrid.prototype.renderHelperSegsInContainers = function (
      pairs,
      sourceSeg
    ) {
      var containerObj,
      ...
      for (j = 0, len = pairs.length; j < len; j++) {
        ...
        for (k = 0, len1 = segs.length; k < len1; k++) {
          ...
          if (
            sourceSeg &&
            sourceSeg.resourceId ===
              ((ref1 = containerObj.resource) != null ? ref1.id : void 0)
          ) {
    ...
    TimelineGrid.prototype.renderFillInContainers = function (
      type,
      pairs,
      className
    ) {
      ...
      for (j = 0, len = pairs.length; j < len; j++) {
        (ref = pairs[j]), (containerObj = ref[0]), (segs = ref[1]);
        results.push(
          this.renderFillInContainer(type, containerObj, segs, className)
        );
      }
      return results;
    };

    TimelineGrid.prototype.renderFillInContainer = function (
      type,
      containerObj,
      segs,
      className
  ...
  TimelineGrid.prototype.initScaleProps = function () {
    ...
    this.headerFormats =
      type === "array"
        ...
        : type === "string"
        ? [input]
        : this.computeHeaderFormats();
    this.isTimeScale = durationHasTime(this.slotDuration);
    this.largeUnit = !this.isTimeScale
      ? ((slotUnit = computeIntervalUnit(this.slotDuration)),
        /year|month|week/.test(slotUnit) ? slotUnit : void 0)
      : void 0;
    return (this.emphasizeWeeks =
      this.slotDuration.as("days") === 1 &&
      this.duration.as("weeks") >= 2 &&
  ...
  ResourceTimelineView = (function (superClass) {
    ...
    ResourceTimelineView.prototype.renderSkeleton = function () {
      ...
      this.joiner = new ScrollJoiner("vertical", [
        this.resourceGrid.bodyScroller,
        this.timeGrid.bodyScroller
      ]);
    ...
    ResourceTimelineView.prototype.renderSkeletonHtml = function () {
      return (
        '<table> <thead class="fc-head"> <tr> <td class="fc-resource-area ' +
        this.widgetHeaderClass +
    ...
    ResourceTimelineView.prototype.initDividerMoving = function () {
      ...
      this.dividerWidth =
        (ref = this.opt("resourceAreaWidth")) != null
          ? ref
          : this.resourceGrid.tableWidth;
      if (this.dividerWidth != null) {
        this.positionDivider(this.dividerWidth);
      }
      return this.dividerEls.on(
        "mousedown",
        (function (_this) {
          return function (ev) {
    ...
    ResourceTimelineView.prototype.queryMiscHeight = function () {
      return (
        this.el.outerHeight() -
        Math.max(
          this.resourceGrid.headScroller.el.outerHeight(),
          this.timeGrid.headScroller.el.outerHeight()
        ) -
        Math.max(
          this.resourceGrid.bodyScroller.el.outerHeight(),
          this.timeGrid.bodyScroller.el.outerHeight()
        )
      );
    };
    ...
    ResourceTimelineView.prototype.syncHeadHeights = function () {
      ...
      headHeight = Math.max(
        this.resourceGrid.headHeight(),
        this.timeGrid.headHeight()
      );
   ...
   */
    ...
    ResourceTimelineView.prototype.reinitializeCellFollowers = function () {
      ...
      this.cellFollower = new ScrollFollower(
        this.resourceGrid.bodyScroller,
        true
      );
    ...
    ResourceTimelineView.prototype.insertResource = function (
      resource,
      parentResourceRow
    ) {
    ...
    ResourceTimelineView.prototype.insertRow = function (
      row,
      parent,
      groupSpecs
    ) {
    ...
    ResourceTimelineView.prototype.computeChildRowPosition = function (
      child,
      parent
    ) {
    ...
    ResourceTimelineView.prototype.ensureResourceGroup = function (
      row,
      parent,
      spec
    ) {
    ...
    ResourceTimelineView.prototype.syncRowHeights = function (
      visibleRows,
      safe
    ) {
    ...
    ResourceTimelineView.prototype.queryScroll = function () {
      ...
      scroll = ResourceTimelineView.__super__.queryScroll.apply(
        this,
        arguments
      );
  ...
  ResourceTimelineGrid = (function (superClass) {
    ...
    ResourceTimelineGrid.prototype.queryHit = function (leftOffset, topOffset) {
      ...
      simpleHit = ResourceTimelineGrid.__super__.queryHit.apply(
        this,
        arguments
      );
      if (simpleHit) {
    ...
    ResourceTimelineGrid.prototype.renderSkeleton = function () {
      ...
      rowContainerEl = $(
        '<div class="fc-rows"><table><tbody/></table></div>'
      ).appendTo(this.bodyScroller.canvas.contentEl);
    ...
    ResourceTimelineGrid.prototype.renderBusinessHours = function () {
      if (this.rowCntWithCustomBusinessHours) {
        ...
        return ResourceTimelineGrid.__super__.renderBusinessHours.apply(
          this,
          arguments
        );
      }
    ...
    ResourceTimelineGrid.prototype.unrenderBusinessHours = function () {
      if (this.rowCntWithCustomBusinessHours) {
        ...
        return ResourceTimelineGrid.__super__.unrenderBusinessHours.apply(
          this,
          arguments
        );
      }
   ...
   */
    ...
    ResourceTimelineGrid.prototype.populateRowBusinessHoursSegs = function (
      row
    ) {
      var businessHourSegs, businessHours, businessHoursEvents;
      businessHours =
        row.resource.businessHours || this.view.opt("businessHours");
      businessHoursEvents = this.view.calendar.computeBusinessHourEvents(
        !this.isTimeScale,
        businessHours
      );
      businessHourSegs = this.eventsToSegs(businessHoursEvents);
      businessHourSegs = this.renderFillSegEls(
        "businessHours",
        businessHourSegs
      );
    ...
    ResourceTimelineGrid.prototype.renderFill = function (
      type,
      segs,
      className
    ) {
  ...
  Spreadsheet = (function () {
    ...
    Spreadsheet.prototype.renderSkeleton = function () {
      ...
      this.headCellEls = this.headScroller.canvas.contentEl.find(
        "tr:last-child th"
      );
    ...
    Spreadsheet.prototype.initColResizing = function () {
      return this.headEl.find("th .fc-col-resizer").each(
        (function (_this) {
    ...
    Spreadsheet.prototype.applyColWidths = function () {
      ...
      for (i = k = 0, len1 = cssWidths.length; k < len1; i = ++k) {
        ...
        tableMinWidth +=
          typeof cssWidth === "number" ? cssWidth : colMinWidths[i];
      }
    ...
    Spreadsheet.prototype.computeColMinWidths = function () {
      ...
      for (i = j = 0, len = ref.length; j < len; i = ++j) {
        ...
        if (typeof width === "number") {
          ...
          results.push(
            parseInt(this.headColEls.eq(i).css("min-width")) || COL_MIN_WIDTH
          );
        }
      }
      return results;
 ...
 */
    ...
    RowParent.prototype.unrender = function () {
      ...
      if (this.isRendered) {
        ...
        for (type in ref) {
          tr = ref[type];
          unrenderMethodName =
            "unrender" + capitaliseFirstLetter(type) + "Content";
   ...
   */
    ...
    RowParent.prototype.animateExpand = function () {
      var ref, ref1, trs;
      trs =
        (ref = this.children[0]) != null
          ? (ref1 = ref.getLeadingRow()) != null
            ? ref1.trs
            : void 0
          : void 0;
      if (trs) {
   ...
   */
    ...
    RowParent.prototype.setTrInnerHeight = function (height) {
      return $.each(
        ...
        (function (_this) {
          return function (type, tr) {
            return getOwnCells(tr)
              .find("> div:not(.fc-cell-content):first")
              .height(height);
          };
        })(this)
      );
 ...
 */
   ...
   */
    ...
    RowGroup.prototype.renderGroupContentEl = function () {
      ...
      contentEl = $('<div class="fc-cell-content" />').append(
        this.renderGroupTextEl()
      );
  ...
  EventRow = (function (superClass) {
    ...
    EventRow.prototype.ensureSegsRendered = function () {
      if (!this.isSegsRendered) {
        ...
        if (this.bgSegs) {
          this.view.timeGrid.renderFillInContainer(
            "bgEvent",
            this,
            this.bgSegs
          );
   ...
   */
    ...
    ResourceRow.prototype.renderSpreadsheetContent = function (tr) {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        td = $('<td class="' + this.view.widgetContentClass + '"/>').append(
          contentEl
        );
        if (colSpec.isMain) {
          td.wrapInner("<div/>");
  ...
  FC.views.agenda.queryResourceClass = function (viewSpec) {
    ...
    if (
      (ref =
        ...
        viewSpec.options.groupByDateAndResource) != null
        ? ref
        : viewSpec.duration.as("days") === 1
    ) {
      return ResourceAgendaView;
    }
  };
